"""Cas d'usage DisableUser — désactivation auditée d'un compte.
...
class DisableUser:
    """Désactive un compte utilisateur, réservé ADMIN/SUPER_ADMIN, audité."""
    ...
    def execute(self, command: DisableUserCommand) -> None:
        """Exécute la désactivation.

        :param command: Données de désactivation.
        ...
        if not context.has_any_role(_ADMIN_ROLES):
            raise ForbiddenError("L'acteur ne peut pas désactiver un compte.")
        ...
        target.disable(self._clock.now())
        with self._uow:
