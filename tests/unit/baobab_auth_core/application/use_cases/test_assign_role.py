"""Tests du cas d'usage AssignRole.
...
class TestAssignRole:
    ...
    def test_BL_030_004_1_assigne_un_role_et_audite(self) -> None:
        # Arrange
        self._seed_roles()
        self.users.save(
        ...
        assert target.role_names == (RoleName("USER"), RoleName("SUPPORT"))
    ...
    def test_BL_030_004_2_super_admin_est_autorise(self) -> None:
        # Arrange
        self._seed_roles()
        self.users.save(
    ...
    def test_BL_030_004_3_assignation_idempotente(self) -> None:
        # Arrange
        self._seed_roles()
        self.users.save(
            self._make_user("actor-1", "subject-admin", (RoleName("ADMIN"),))
        ...
        target = self.users.get_by_id(UserId("target-1"))
        assert target is not None
        assert target.role_names == (RoleName("SUPPORT"),)
        assert self.audit.all_events == []
        assert not self.uow.committed

    def test_BL_030_004_4_acteur_non_autorise_refuse(self) -> None:
        # Arrange
        self._seed_roles()
    ...
    def test_BL_030_004_5_acteur_inconnu_refuse(self) -> None:
        # Arrange
        self._seed_roles()
        self.users.save(self._make_user("target-1", "subject-target", ()))
    ...
    def test_BL_030_004_6_cible_inconnue_refusee(self) -> None:
        # Arrange
        self._seed_roles()
        self.users.save(
            self._make_user("actor-1", "subject-admin", (RoleName("ADMIN"),))
    ...
    def test_BL_030_004_7_role_inconnu_refuse(self) -> None:
        # Arrange
        self._seed_roles()
        self.users.save(
            self._make_user("actor-1", "subject-admin", (RoleName("ADMIN"),))
