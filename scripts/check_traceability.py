...
def _subdirs(directory: Path) -> list[Path]:
    ...
    if not directory.exists():
        return []
    return [d for d in directory.iterdir() if d.is_dir() and not d.name.startswith(".")]
...
def check_queue_entries(errors: list[str]) -> None:
    ...
    for entry in queue:
        ...
        if not fiche.exists():
            errors.append(
                f"[{bl_id}] fiche manquante : " f"docs/backlog/backlogs/{bl_id}.md"
            )
            continue
...
def main() -> int:
    ...
    if errors:
        ...
        print(
            "\nLa chaîne besoin → US/FEAT → backlog → run → ADR" " doit être complète.",
            file=sys.stderr,
        )
        return 1
